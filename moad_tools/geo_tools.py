# Copyright 2019 The UBC EOAS MOAD Group
...
def make_mapping_file(coordinate_file, mapping_file,
                      grid_description, lon_var='nav_lon', lat_var='nav_lat'):
    """Make two arrays that index from longitude and latitude to grid index
       ...
       :param str grid_description: name of the grid that is being mapped, to be                                    written as a comment into the netcdf file
    ...
    dlon = max(abs(nav_lons[1, 1] - nav_lons[0, 0]),
               abs(nav_lons[0, 1] - nav_lons[1, 0]))/4.
    dlat = max(abs(nav_lats[1, 1] - nav_lats[0, 0]),
               abs(nav_lats[0, 1] - nav_lats[1, 0]))/4.
    ...
    # set index arrays
    iis = np.ndarray.flatten(np.tile(np.arange(dimensions[0]),
                                     (dimensions[1], 1)).transpose())
    jjs = np.ndarray.flatten(np.tile(np.arange(dimensions[1]),
                                     (dimensions[0], 1)))
    # grid the indexes on the lon/lat grid
    indexi = interpolate.griddata((nav_lons, nav_lats), iis,
                                  (lons, lats), method='nearest').astype(int)
    indexj = interpolate.griddata((nav_lons, nav_lats), jjs,
                                  (lons, lats), method='nearest').astype(int)
    ...
    da = {}
    da['i'] = xr.DataArray(
            data=indexi, name='Value of I index', dims=dims,
            coords=coords, attrs=attrs)
    da['j'] = xr.DataArray(
            data=indexj, name='Value of J index', dims=dims,
            coords=coords, attrs=attrs)
    da['dlon'] = xr.DataArray(
            data=dlon, name='grid size in longitude',
        attrs={'units': 'Degrees Longitude'})
    da['dlat'] = xr.DataArray(
            data=dlat, name='grid size in latitude',
        attrs={'units': 'Degrees Latitude'})
    da['lonmin'] = xr.DataArray(
            data=lonmin, name='minimum longitude value',
        attrs={'units': 'Degrees Longitude'})
    da['latmin'] = xr.DataArray(
            data=latmin, name='minimum latitude value',
        attrs={'units': 'Degrees Latitude'})
    data_vars = {'indexi': da['i'],
            'indexj': da['j'],
            'dlon': da['dlon'],
            'dlat': da['dlat'],
            'lonmin': da['lonmin'],
    ...
    ds_attrs = {
        ...
        'summary': (
            'Mapping from lons and lats to ' + grid_description + ' grid'
        ),
        'source': (
            'http:/bitbucket.org/UBC_MOAD/moad_tools/grid_tools.py'
        ),
        'history': (
            '[{}] File creation.'
